...
get_waypoints = function()
{
	...
	if (roomWeights[inRoom.numberRoom] != 0)
	{
		...
		do
		{
			...
			_checkRoom = _waypoints[_numberWaypoint].room;
			_numberWaypoint++;
		}
		until (_lowest == 0)
	}
...
stateExecutable.run = function()
{
	...
	if (distance_to_object(obj_player) <= rangeFinisher)
	{
		...
		if (keyboard_check_pressed(ord("F"))) instance_create_layer(x, y, "Overlay", obj_managerMinigame, { game : "finisher", enemy : id });
	}
}
